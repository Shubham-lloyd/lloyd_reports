...
AND a.tran_date >= last_day(add_months(TO_DATE(sysdate), - 2)) + 1
AND a.tran_date <= add_months(TO_DATE(sysdate), - 1)
        and a.site_code like 'S%'
        and a.site_code not in ('S0001', 'S0003')


		AND NOT EXISTS (SELECT 1 FROM sreturn sr WHERE sr.invoice_id = a.invoice_id AND confirmed = 'Y' AND tran_date <= add_months(TO_DATE(sysdate), - 1) AND tran_type = 'IC')
...
AND a.invoice_id >= '0'
AND a.invoice_id <= 'Z'
AND a.tran_date >= last_day(add_months(TO_DATE(sysdate), - 1)) + 1
AND a.tran_date <= TO_DATE(sysdate)
        and a.site_code like 'S%'
        and a.site_code not in ('S0001', 'S0003')

AND NOT EXISTS (SELECT 1 FROM sreturn sr WHERE sr.invoice_id = a.invoice_id AND confirmed = 'Y' AND tran_date <= TO_DATE(sysdate) AND tran_type = 'IC')
...
--,s.state_code AS state_code
 ,case when s.site_code in ('S0005' , 'S0007') then '1-CWH (VB)' else s.state_code end state_code
...
SELECT TO_CHAR(TO_DATE(sysdate), 'dd.MM.YY') AS AS_ON_DATE,
    (CASE WHEN state_code IS NULL THEN TO_CHAR(TO_DATE(sysdate), 'ddth Mon-YY') ELSE state_code END) state_code
...
FROM (
    ...
    ,SUM(gross_sales_tr06) AS gross_sales_tr06
		...
		,SUM(os_amt) AS os_amt
	FROM (
	SELECT state_code
			,gross_sales_tr01 AS gross_sales_tr01
...
,(PRI_NET_SALES_TR06 - (COMPNY_DISC_TR06)) as PRI_NET_SALES_TR06
			...
			,0 AS os_amt
		FROM tab3
		WHERE tran_date >= last_day(add_months(TO_DATE(sysdate), - 1)) + 1
			AND tran_date <= TO_DATE(sysdate)

		UNION ALL

SELECT  case when s.site_code in ('S0005' , 'S0007') then '1-CWH (VB)' else s.state_code end state_code
		-- s.state_code
			,0 AS gross_sales_tr01
 ...
 ,0 AS gross_sales_tr06
			...
			,(CASE WHEN (pr.ref_date = TO_DATE(sysdate) AND NOT EXISTS (SELECT 1 FROM receipt WHERE tran_id = tran_ref_rcp AND confirmed = 'Y')) THEN pr.amount ELSE 0 END) AS chq_dep_val
			,pr.amount AS tot_dep_val
			,0 As tot_rcp_val
			,0 AS os_amt
		FROM pdc_received pr
			,site s
		WHERE (pr.site_code = s.site_code)
			AND pr.STATUS IN ('P')
            and s.site_code like 'S%'
            and s.site_code not in ('S0001', 'S0003')

			AND (
				pr.ref_date >= last_day(add_months(TO_DATE(sysdate), - 1)) + 1
				AND pr.ref_date <= TO_DATE(sysdate)
				)
...
,0 AS gross_sales_tr06
		...
		FROM Receipt r
			,site s
		WHERE (r.site_code = s.site_code)
		And Confirmed = 'Y'
        and s.site_code like 'S%'
        and s.site_code not in ('S0001', 'S0003')
		And Tran_Date >= last_day(add_months(TO_DATE(sysdate), - 1)) + 1
		And Tran_Date <= TO_DATE(sysdate)
		And Not Exists (Select 1 from rcpdishnr rd Where Confirmed = 'Y' And rd.Receipt_No = r.Tran_Id)

		UNION ALL

		SELECT  case when site.site_code in ('S0005' , 'S0007') then '1-CWH (VB)' else site.state_code end state_code
		-- site.state_code
			,0 AS gross_sales_tr01
			,0 AS sales_return_tr01
            ,0 as PRI_NET_SALES_TR01
			...
			,0 AS tot_dep_val
			,0 As tot_rcp_val
			,(receivables.tot_amt - receivables.adj_amt) AS os_amt
		FROM receivables
			,site
		WHERE (receivables.site_code = site.site_code)
        and site.site_code like 'S%'
        and site.site_code not in ('S0001', 'S0003')
		) fin_roll
	GROUP BY ROLLUP(state_code)

	UNION ALL

	SELECT TO_CHAR(add_months(TO_DATE(sysdate), - 1), 'ddth Mon-YY') state_code
		,SUM(gross_sales_tr01) AS gross_sales_tr01
		,SUM(sales_return_tr01) AS sales_return_tr01
		,SUM(PRI_NET_SALES_TR01) AS PRI_NET_SALES_TR01
		,SUM(gross_sales_tr02) AS gross_sales_tr02
...
,(PRI_NET_SALES_TR06 - (COMPNY_DISC_TR06)) as PRI_NET_SALES_TR06
			...
			,0 AS os_amt
		FROM tab3
		WHERE tran_date >= last_day(add_months(TO_DATE(sysdate), - 2)) + 1
			AND tran_date <= add_months(TO_DATE(sysdate), - 1)
		...
		SELECT 0 AS gross_sales_tr01
			...
			,0 AS sales_return_tr06
            ,0 as PRI_NET_SALES_TR06
			,(CASE WHEN (pr.ref_date = add_months(TO_DATE(sysdate), - 1) AND NOT EXISTS (SELECT 1 FROM receipt WHERE tran_id = tran_ref_rcp AND confirmed = 'Y')) THEN pr.amount ELSE 0 END) AS chq_dep_val
			,pr.amount AS tot_dep_val
			,0 As tot_rcp_val
			,0 AS os_amt
		FROM pdc_received pr
		WHERE pr.STATUS IN ('P')
			AND (
				pr.ref_date >= last_day(add_months(TO_DATE(sysdate), - 2)) + 1
				AND pr.ref_date <= add_months(TO_DATE(sysdate), - 1)
				)
			--And Not Exists (Select 1 from receipt where tran_id = tran_ref_rcp And Confirmed = 'Y')
			AND pr.amount <> 0
  ...
  ,0 AS gross_sales_tr06
		...
		FROM Receipt r
		WHERE Confirmed = 'Y'
		And Tran_Date >= last_day(add_months(TO_DATE(sysdate), - 2)) + 1
		And Tran_Date <= add_months(TO_DATE(sysdate), - 1)
		And Not Exists (Select 1 from rcpdishnr rd Where Confirmed = 'Y' And rd.Receipt_No = r.Tran_Id)
		...
		SELECT 0 AS gross_sales_tr01
			...
			,0 AS tot_dep_val
			,0 As tot_rcp_val
			,fn_rrecv_rep_fld(receivables.tran_ser, receivables.ref_no, add_months(TO_DATE(sysdate), - 1), receivables.tot_amt - receivables.adj_amt) AS os_amt
		FROM receivables
		WHERE (receivables.tran_date <= add_months(TO_DATE(sysdate), - 1))
        and site_code like 'S%'
        and site_code not in ('S0001', 'S0003')

			AND (
				(
					add_months(TO_DATE(sysdate), - 1) BETWEEN receivables.ref_date
						AND fn_radd_days(receivables.stat_date, - 1)
					)
				OR (tot_amt <> adj_amt)
				)
		) prev_mon
	GROUP BY TO_CHAR(add_months(TO_DATE(sysdate), - 1), 'ddth Mon-YY')
	)
 )TAB12